...
RSpec.describe 'show page' do
  ...
  it 'shows the dish name and description, ingredients, and chef name' do
    visit "/dishes/#{@spag.id}"
    expect(page).to have_content(@spag.name)
    expect(page).to have_content(@spag.description)
    ...
    expect(page).to have_content(@spag.chef.name)
  end

  it 'should have a calorie count for the dish' do
    visit "/dishes/#{@spag.id}"
    expect(page).to have_content("580 calories")
